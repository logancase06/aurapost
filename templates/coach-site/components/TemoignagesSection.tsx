...
export default function TemoignagesSection({ data, style, accent, t }: { data: CoachSiteData; style: SiteStyle; accent: string; t: Theme }) {
  ...
  const sectionBgByStyle: Record<SiteStyle, string> = { impact: '#09090b', clarte: '#F9FAFB', authenticite: '#F5F5F4' };
  const cardBgByStyle: Record<SiteStyle, string> = { impact: '#141414', clarte: '#FFFFFF', authenticite: '#FFFFFF' };
  const cardRadiusByStyle: Record<SiteStyle, number> = { impact: 4, clarte: 16, authenticite: 16 };
  const sectionBg = sectionBgByStyle[style];
  const cardBg = cardBgByStyle[style];
  const cardRadius = cardRadiusByStyle[style];
    ...
    <section id="temoignages" style={{ background: sectionBg, color: t.ink }}>
      <Reveal style={{ maxWidth: 1080, margin: '0 auto', padding: '100px 24px' }}>
        ...
        <div style={{ maxWidth, margin: items.length === 1 ? '0 auto' : undefined, display: 'grid', gridTemplateColumns: cols, gap: 24 }}>
          {items.map((q, i) => (
            <figure key={i} style={{ position: 'relative', margin: 0, overflow: 'hidden', background: cardBg, border: `1px solid ${t.border}`, borderRadius: cardRadius, padding: '32px 28px 28px' }}>
